...
| Vite dev server: top-level `await` in client modules transitively imported into a Pages Functions backend | Breaks HMR silently (page renders empty, no console error) but the production build still works. Put async helpers inside functions, never at module top level. When in doubt, verify with `npm run preview` of the built `dist/` — that's the ground truth. |
